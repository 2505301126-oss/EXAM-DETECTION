document.addEventListener("DOMContentLoaded", () => {
    ...
    sendOtpBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                emailError.innerHTML = `${data.message || "Failed to send OTP."}<br><small>Ensure EMAIL_USER/PASS are set in .env</small>`;
            }
        } catch (error) {
            console.error("Auth Error:", error);
            emailError.innerHTML = "Backend server offline.<br><small>Run 'node server.js' to enable email OTP.</small>";
        } finally {
    ...
    verifyOtpBtn.addEventListener("click", async () => {
        ...
        if (!otpValue) { otpError.textContent = "Please enter the code sent to your email."; return; }
        ...
        try {
            ...
            otpError.textContent = "Verification failed. Check server connection.";
            verifyOtpBtn.textContent = "Verify & Login";
            verifyOtpBtn.disabled = false;
        }
    });
